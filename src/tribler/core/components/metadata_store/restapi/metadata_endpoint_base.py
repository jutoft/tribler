...
class MetadataEndpointBase(RESTEndpoint):
    ...
    def sanitize_parameters(cls, parameters):
        ...
        sanitized = {
            ...
            "sort_desc": parse_bool(parameters.get('sort_desc', 1)),
            "txt_filter": parameters.get('txt_filter'),
            "hide_xxx": parse_bool(parameters.get('hide_xxx', 0)),
            "category": parameters.get('category'),
            "exclude_deleted": parse_bool(parameters.get('exclude_deleted', 0)),
        }
        if 'tags' in parameters:
            sanitized['tags'] = parameters.getall('tags')
        if "remote" in parameters:
            sanitized["remote"] = (parse_bool(parameters.get('remote', 0)),)
